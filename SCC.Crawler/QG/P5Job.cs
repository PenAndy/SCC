...
namespace SCC.Crawler.QG
{
    ...
    public class P5Job : IJob
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            LatestItem = context.JobDetail.JobDataMap["LatestItem"] as OpenCodePL5TModel;
            try
            {
                //服务启动时配置初始数据
                if (LatestItem == null)
                {
                    LatestItem = services.GetOpenCodePL5TLastItem(currentLottery);
                    if (LatestItem == null)
                        LatestItem = new OpenCodePL5TModel
                        {
        ...
        private void DoMainUrl()
        {
            if (!string.IsNullOrEmpty(Config.MainUrl))
            {
                ...
                for (var i = startQiNum; i <= newestQiHao; i++)
                {
                    ...
                    if (matchItem != null && services.AddDTOpenPL5Code(currentLottery, matchItem))
                    {
                        //Do Success Log
        ...
        {
            var url = "https://www.8200.cn/kjh/p5/" + optimizeUrl + ".htm";
        ...
        ///     当天抓取的最新一期开奖记录
        /// </summary>
        private OpenCodePL5TModel LatestItem;
