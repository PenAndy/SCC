    ...
    public class GD11X5Job : IJob
    {
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            var requestUrl = Config.MainUrl +
                             string.Format("&date={0}", CommonHelper.SCCSysDateTime.ToString("yyyy-MM-dd"));
        ...
        {
            var requestUrl = Config.MainUrl + string.Format("&date={0}",
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(string url, bool IsToday = true)
        {
            ...
            try
            {
                ...
                for (var i = 2; i < trs.Count; i++) //第一行是日期，第二行是表头
                {
                    ...
                    if (tds.Count < 46) continue;
                    if (tds[0].InnerText.Trim() == "合计期")
                        break;
                    matchQiHao = tds[0].InnerText.Trim();
                    matchKJHaoMa = tds[1].InnerText.Trim().Replace('，', ',');
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.GuangDong11X5),
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.GuangDong11X5;
