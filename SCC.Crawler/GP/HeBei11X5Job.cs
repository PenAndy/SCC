...
namespace SCC.Crawler.GP
{
    ...
    public class HeBei11X5Job : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        ...
        /// <param name="IsToday"></param>
        /// <returns></returns>
        private List<HeBei11X5Entity> GetDocByMainUrl(bool IsToday = true)
        {
            var list = new List<HeBei11X5Entity>();
            try
            {
                var time = DateTime.Now;
                var arg = new OpenCaiApiArg
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.HeBei11X5),
        ...
        /// <returns></returns>
        private bool SaveRecord(HeBei11X5Entity data)
        {
        ...
        #region   爬取爱彩乐网站数据
        private List<HeBei11X5Entity> GetDocByBackUrl(bool IsToday = true)
        {
            var list = new List<HeBei11X5Entity>();
            try
            {
                var day = "?op=dcjb&num=jt";
                if (!IsToday) day = "?op=dcjb&num=zt";
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.HeBei11X5;
