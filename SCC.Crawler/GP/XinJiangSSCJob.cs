...
namespace SCC.Crawler.GP
{
    ...
    public class XinJiangSSCJob : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        public XinJiangSSCJob()
        {
            log = new LogHelper();
        ...
        /// <returns></returns>
        private List<XinJiangSSCEntity> GetDocByBackUrl(bool IsToday = true)
        {
            var list = new List<XinJiangSSCEntity>();
            try
            {
                ...
                var HtmlResource =
                    NetHelper.GetUrlResponse(Config.BackUrl, "POST", string.Format("selectDate={0}", day));
                ...
                {
                    var tmp = new XinJiangSSCEntity
                    {
                        Id = int.Parse(obj[i]["lotteryIssue"].ToString().Substring(2)),
                        OpenCode = obj[i]["lotteryNumber"].ToString(),
                        OpenTime = CommonHelper.UnixTimeStamp(long.Parse(obj[i]["annoucementTime"]["time"].ToString()
                            .Substring(0, obj[i]["annoucementTime"]["time"].ToString().Length - 3)))
                    };
                    list.Add(tmp);
                }
        ...
        /// <param name="IsToday"></param>
        /// <returns></returns>
        private List<XinJiangSSCEntity> GetDocByMainUrl(bool IsToday = true)
        {
            var list = new List<XinJiangSSCEntity>();
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.XinJiangSSC),
        ...
        /// <returns></returns>
        public bool SaveRecord(XinJiangSSCEntity data)
        {
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.XinJiangSSC;
    ...
    ///     开奖号实体
    /// </summary>
    public class XinJiangSSCEntity
    {
