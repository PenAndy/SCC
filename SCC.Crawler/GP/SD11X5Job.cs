    ...
    public class SD11X5Job : IJob
    {
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.ShanDong11X5),
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            ...
            try
            {
                ...
                if (tables == null || tables.Count < 15) return result;
                var trs = tables[14].ChildNodes.Where(node => node.Name == "tr").ToList();
                var matchQiHao = string.Empty;
                var matchKJHaoMa = string.Empty;
                for (var i = 2; i < trs.Count; i++) //第一二行是表头
                {
                    ...
                    matchKJHaoMa = string.Format("{0},{1},{2},{3},{4}", tds[1].InnerText.Trim(),
        ...
        private Dictionary<string, string> GetYesterdayOpenListFromBackUrl()
        {
            ...
            try
            {
                var requestUrl = Config.BackUrl + "?page={0}";
                var HtmlResource = string.Empty;
                HtmlNodeCollection tables = null;
                List<HtmlNode> trs, tds = null;
                var matchQiHao = string.Empty;
                var matchKJHaoMa = string.Empty;
                var todayQiHaoString = CommonHelper.SCCSysDateTime.ToString("yyMMdd");
                var daybeforeyesterdayQiHaoString = CommonHelper.SCCSysDateTime.AddDays(-2).ToString("yyMMdd");
                for (var k = 1; k <= 7; k++)
                {
                    HtmlResource = NetHelper.GetUrlResponse(string.Format(requestUrl, k));
                    if (string.IsNullOrWhiteSpace(HtmlResource)) continue;
                    var doc = new HtmlDocument();
                    doc.LoadHtml(HtmlResource);
                    tables = doc.DocumentNode.SelectNodes("//table");
                    if (tables.Count < 15) continue;
                    trs = tables[14].ChildNodes.Where(node => node.Name == "tr").ToList();
                    for (var i = 2; i < trs.Count; i++) //第一二行是表头
                    {
                        var trstyle = trs[i].Attributes["style"];
                        if (trstyle != null && trstyle.Value == "display:none") continue;
                        ...
                        if (tds.Count <= 1) continue;
                        matchQiHao = tds[0].InnerText.Trim();
                        if (matchQiHao.StartsWith(todayQiHaoString)) continue;
                        if (matchQiHao.StartsWith(daybeforeyesterdayQiHaoString)) return result;
                        matchKJHaoMa = string.Format("{0},{1},{2},{3},{4}", tds[1].InnerText.Trim(),
                            tds[2].InnerText.Trim(), tds[3].InnerText.Trim(), tds[4].InnerText.Trim(),
                            tds[5].InnerText.Trim());
                        result.Add(matchQiHao, CommonHelper.TRHandleCode(matchKJHaoMa));
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.ShanDong11X5;
