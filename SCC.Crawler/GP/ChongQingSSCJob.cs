...
namespace SCC.Crawler.GP
{
    ...
    public class ChongQingSSCJob : IJob
    {
        public ChongQingSSCJob()
        {
            log = new LogHelper();
        ...
        /// <returns></returns>
        private List<ChongQingSSCEntity> GetDocByBackUrl(bool IsToday = true)

        {
            var list = new List<ChongQingSSCEntity>();
            try
            {
                ...
                var tables = doc.DocumentNode.SelectNodes("//*[@id=\"mainArea\"]/div[1]/table");
                if (tables == null) return list;

                foreach (var item in tables[0].ChildNodes)
                foreach (var item2 in item.ChildNodes)
                {
                    if (item2.GetAttributeValue("class", "") == "start" && item2.Name == "td")
                    {
                        var num = item2.GetAttributeValue("data-period", "");
                        var opencode = item2.GetAttributeValue("data-win-number", "");
                        if (!string.IsNullOrEmpty(opencode))
                        {
                            var tmp = new ChongQingSSCEntity
                            {
                                Id = int.Parse(num),
                    ...
                    {
                        if (item3.GetAttributeValue("class", "") == "start" && item3.Name == "td")
                        {
                            var num2 = item3.GetAttributeValue("data-period", "");
                            var opencode2 = item3.GetAttributeValue("data-win-number", "");
                            if (!string.IsNullOrEmpty(opencode2))
                            {
                                var tmp2 = new ChongQingSSCEntity
                                {
                                    Id = int.Parse(num2),
                                    OpenCode = opencode2.Replace(" ", ","),
                                    OpenTime = GetOpenTimeByPeriodNum(int.Parse(num2))
                                };
                                list.Add(tmp2);
                            }
                        }
                        ...
                        foreach (var item4 in item3.ChildNodes)
                            if (item4.GetAttributeValue("class", "") == "start" && item4.Name == "td")
                            {
                                var num3 = item4.GetAttributeValue("data-period", "");
                                var opencode3 = item4.GetAttributeValue("data-win-number", "");
                                if (!string.IsNullOrEmpty(opencode3))
                                {
                                    var tmp3 = new ChongQingSSCEntity
                                    {
                                        Id = int.Parse(num3),
                                        OpenCode = CommonHelper.TRHandleCode(opencode3.Replace(" ", ",")),
                                        OpenTime = GetOpenTimeByPeriodNum(int.Parse(num3))
                                    };
                                    list.Add(tmp3);
                                }
                            }
        ...
        /// <param name="IsToday"></param>
        /// <returns></returns>
        private List<ChongQingSSCEntity> GetDocByMainUrl(bool IsToday = true)
        {
            var list = new List<ChongQingSSCEntity>();
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.ChongQingSSC),
        ...
        /// <returns></returns>
        public bool SaveRecord(ChongQingSSCEntity data)
        {
        ...
        public DateTime GetOpenTimeByPeriodNum(int PeriodNum)
        {
            ...
            if (qi <= 72)
                time = time.AddMinutes(qi * 10);
            else
                time = time.AddMinutes(5 * (qi - 72) + 10 * 72);
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.ChongQingSSC;
    ...
    ///     开奖号实体
    /// </summary>
    public class ChongQingSSCEntity
    {
