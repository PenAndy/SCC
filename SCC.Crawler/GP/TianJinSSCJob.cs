...
namespace SCC.Crawler.GP
{
    ...
    public class TianJinSSCJob : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        public TianJinSSCJob()
        {
            log = new LogHelper();
        ...
        /// <returns></returns>
        private List<TianJinSSCEntity> GetDocByBackUrl(bool IsToday = true)

        {
            var list = new List<TianJinSSCEntity>();
            try
            {
                var param = "op=jb5&num={0}";
                var day = "j1";
                if (!IsToday) day = "j2";
                var url = Config.BackUrl + string.Format(param, day);
                var HtmlResource = NetHelper.GetUrlResponse(url);
                if (HtmlResource == null) return list;

                var doc = new HtmlDocument();
                doc.LoadHtml(HtmlResource);
                var tables = doc.DocumentNode.SelectNodes("//*[@id=\"content\"]/table");
                ...
                foreach (var item in tables[0].ChildNodes)
                {
                    ...
                    foreach (var item2 in item.ChildNodes)
                    {
                        ...
                        if (item2.GetAttributeValue("class", "") == "chart-bg-qh") num = item2.InnerText;
                        if (item2.GetAttributeValue("class", "1") == "1")
                            opencode = item2.InnerText.Length == 9 ? item2.InnerText : "";
                        if (opencode.Length == 9)
                        {
                            var tmp = new TianJinSSCEntity
                            {
                                Id = int.Parse(num.Substring(2)),
                                OpenCode = opencode,
                                OpenTime = GetOpenTimeByPeriodNum(int.Parse(num.Substring(2)))
                            };
                            list.Add(tmp);
                            break;
                        }
        ...
        /// <param name="IsToday"></param>
        /// <returns></returns>
        private List<TianJinSSCEntity> GetDocByMainUrl(bool IsToday = true)
        {
            var list = new List<TianJinSSCEntity>();
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.TianJinSSC),
        ...
        /// <returns></returns>
        public bool SaveRecord(TianJinSSCEntity data)
        {
        ...
        public string GetPeriodsNumberToDay(DateTime time, bool IsToday = false)
        {
            ...
            int hc = 0, mc = 0;
            if (time.Hour >= Config.StartHour) //1
            {
                hc = (time.Hour - Config.StartHour) * 4; //已开期数小时部分
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.TianJinSSC;
