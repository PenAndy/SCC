...
namespace SCC.Crawler.GP
{
    ...
    public class HeBeiK3Job : IJob
    {
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            ...
            try
            {
                var postdata = string.Format("selectDate={0}", CommonHelper.SCCSysDateTime.ToString("yyyyMMdd"));
                var HtmlResource = NetHelper.GetUrlResponse(Config.BackUrl, "POST", postdata);
                if (HtmlResource == null) return result;

                var array = HtmlResource.JsonToEntity<JArray>();
                if (array != null && array.Count > 0)
                {
                    ...
                    {
                        matchQiHao = CommonHelper.SCCSysDateTime.ToString("yyMMdd") +
                                     item["lotteryIssue"].ToString().Trim().PadLeft(3, '0');
                        matchKJHaoMa = item["lotteryNumber"].ToString().Trim();
        ...
        private Dictionary<string, string> GetYesterdayOpenListFromBackUrl()
        {
            ...
            try
            {
                var postdata = string.Format("selectDate={0}",
                ...
                if (!string.IsNullOrWhiteSpace(HtmlResource))
                {
                    ...
                    if (array != null && array.Count > 0)
                    {
                        ...
                        foreach (var item in array)
                        {
                            matchQiHao = CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyMMdd") +
                                         item["lotteryIssue"].ToString().Trim().PadLeft(3, '0');
                            matchKJHaoMa = item["lotteryNumber"].ToString().Trim();
                            if (!result.ContainsKey(matchQiHao))
                                result.Add(matchQiHao, CommonHelper.TRHandleCode(matchKJHaoMa));
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.HeBeiK3),
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.HeBeiK3;
