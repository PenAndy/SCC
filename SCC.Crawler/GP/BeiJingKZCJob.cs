...
namespace SCC.Crawler.GP
{
    ...
    public class BeiJingKZCJob : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        ...
        private void DoTodayJobByBackUrl()
        {
            if (!string.IsNullOrEmpty(Config.BackUrl))
            {
                ...
                var newestQiNum = Convert.ToInt32(newestQiHao.Substring(5));
        ...
        /// <returns></returns>
        private List<BeiJingKZCEnitity> GetDocByBackUrl(bool IsToday = true)
        {
            var list = new List<BeiJingKZCEnitity>();
            try
            {
                var day = DateTime.Now.ToString("yyyyMMdd");
                if (!IsToday) day = DateTime.Now.AddDays(-1).ToString("yyyyMMdd");
                var url = string.Format(Config.BackUrl, day);
                //" http://kaijiang.500.com/static/info/kaijiang/xml/bjkzhc/20171124.xml?_A=YLQMBVJT1511764350898"; 
        ...
        /// <returns></returns>
        private List<BeiJingKZCEnitity> GetDocByMainUrl(bool IsToday = true)
        {
            var list = new List<BeiJingKZCEnitity>();
            try
            {
        ...
        /// <returns></returns>
        private bool SaveRecord(BeiJingKZCEnitity data)
        {
            if (data == null) return false;

            var model = new OpenCode9Model();
            ...
            model.OpenCode9 = Convert.ToInt32(haoMaArray[8]);
            model.OpenTime = data.OpenTime;
            if (services.AddOpen9Code(currentLottery, model))
            {
        ...
        ///     数据服务
        /// </summary>
        private readonly IOpen9Code services;

        /// <summary>
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.BeiJingKZC;
    ...
    public class BeiJingKZCEnitity
    {
