    ...
    public class JX11X5Job : IJob
    {
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.JiangXi11X5),
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            ...
            try
            {
                ...
                for (var i = 1; i < trs.Count; i++) //第一行是是表头
                {
                    ...
                    temp = tds[2].InnerText.Trim().Replace("\r\n", "").Replace(" ", "");
                    if (temp.Length != 10) continue;
                    matchKJHaoMa = temp.Insert(8, ",").Insert(6, ",").Insert(4, ",").Insert(2, ",");
        ...
        private Dictionary<string, string> GetYesterdayOpenListFromBackUrl()
        {
            ...
            try
            {
                var HtmlResource = NetHelper.GetUrlResponse(Config.BackUrl, "POST",
                    string.Format("riqi={0}&x=47&y=11", CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyyy-MM-dd")),
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.JiangXi11X5;
