...
namespace SCC.Crawler.GP
{
    ...
    public class ShanXiTaiYuanKL10FJob : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        public ShanXiTaiYuanKL10FJob()
        {
            log = new LogHelper();
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(bool IsToday = true)
        {
            ...
            try
            {
                var Url = Config.BackUrl + "/?op=dcjb&num=500";
                ...
                if (tables.Count < 3) return result;
                var trs = tables[2].ChildNodes.Where(node => node.Name == "tr").ToList();
                var sun = trs.Count - 13;
                var matchQiHao = string.Empty;
                var matchKJHaoMa = string.Empty;
                var tds = new List<HtmlNode>();
                for (var i = 2; i <= sun; i++) //第一二行是表头
                {
                    ...
                    matchKJHaoMa = string.Format("{0},{1},{2},{3},{4},{5},{6},{7}", tds[1].InnerText.Trim(),
                        tds[2].InnerText.Trim(), tds[3].InnerText.Trim(), tds[4].InnerText.Trim(),
                        tds[5].InnerText.Trim(), tds[6].InnerText.Trim(), tds[7].InnerText.Trim(),
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.ShanXiTaiYuanKL10F),
        ...
        private bool SaveRecord(string QiHao, string OpenCode, bool IsYesterdayRecord)
        {
            if (!string.IsNullOrWhiteSpace(QiHao) && !string.IsNullOrWhiteSpace(OpenCode))
            {
                ...
                return services.AddOpen8Code(currentLottery, model);
            }
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.ShanXiTaiYuanKL10F;
