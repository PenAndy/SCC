    ...
    public class SHSSLJob : IJob
    {
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            ...
            try
            {
                ...
                var divs = doc.DocumentNode.SelectNodes("//div").ToList();
                var targetDiv = divs
                    .Where(R => R.Attributes["class"] != null && R.Attributes["class"].Value == "lm2block1")
                    .FirstOrDefault();
                if (targetDiv == null) return result;
                var childDivs = targetDiv.ChildNodes.Where(R => R.Name == "div").ToList();
                if (childDivs.Count >= 4 && (childDivs.Count - 1) % 3 == 0) //第一个div为表头，后面每3个div一期开奖记录
                {
                    var matchQiHao = string.Empty;
                    var matchKJHaoMa = string.Empty;
                    for (var i = 0; i < childDivs.Count / 3; i++)
                    {
                        matchQiHao = childDivs[i * 3 + 1].InnerText.Replace("-", string.Empty).Substring(2);
                        matchKJHaoMa = childDivs[i * 3 + 3].InnerText.Replace(" ", string.Empty).Insert(2, ",")
                            .Insert(1, ",");
        ...
        private Dictionary<string, string> GetYesterdayOpenListFromBackUrl()
        {
            ...
            try
            {
                var HtmlResource = NetHelper.GetUrlResponse(Config.BackUrl, "POST",
                    string.Format("dateOptions={0}", CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyyy-MM-dd")));
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.ShangHaiSSL),
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.ShangHaiSSL;
