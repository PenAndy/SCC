    ...
    public class GXKL10FJob : IJob
    {
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(LatestQiHao) || !CheckLatestQiHaoIsTodayTerm())
                {
                    CheckingYesterdayTheLotteryData();
                    LatestQiHao = CommonHelper.GenerateGXKL10FQiHao(CommonHelper.SCCSysDateTime, Config.SkipDate, 0);
                }

                //当最新期号不符合当天总期数，执行当天作业
                if (Convert.ToInt32(LatestQiHao.Substring(7)) != Config.TimesPerDay)
                {
                    DoTodayJobByMainUrl();
                    DoTodayJobByBackUrl();
                }
        ...
        private void CheckingYesterdayTheLotteryData()
        {
            ...
            //从数据库中获取昨天数据抓取失败列表
            FailedQiHaoList =
                services.GetGXKL10FYesterdayFailQQList(currentLottery, Config.SkipDate, Config.TimesPerDay);
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.GuangXiKL10F),
                ...
                if (data.data != null)
                {
                    ...
                    for (var i = 0; i < data.data.Count; i++)
                        result.Add(data.data[i].expect, data.data[i].GetOpenCodeStr());
        ...
        {
            var requestUrl = string.Format(Config.BackUrl, CommonHelper.SCCSysDateTime.ToString("yyyyMMdd"));
        ...
        {
            var requestUrl =
                string.Format(Config.BackUrl, CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyyyMMdd"));
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(string url, bool IsToday = true)
        {
            ...
            try
            {
                ...
                foreach (var item in tables[0].ChildNodes)
                {
                    ...
                    {
                        if (item2.Name == "th") break;
                        if (item2.Name == "td")
                            foreach (var item3 in item2.ChildNodes)
                            {
                                if (item3.Name == "span")
                                    qihao = item3.InnerText.Trim().Substring(0, item3.InnerText.Trim().Count() - 5);
                                if (item3.Name == "i")
                                {
                                    var tmpcode = item3.InnerText;
                                    opencode.Add(tmpcode);
                                }
                            }
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public string GetPeriodsNumberByTime(DateTime time)
        {
            double mc = 0;
            var rt = 0;
            if (time.Hour >= Config.StartHour && time.Minute > Config.StartMinute)
            {
                mc = (time - new DateTime(time.Year, time.Month, time.Day, Config.StartHour, Config.StartMinute, 0))
                     .TotalMinutes / Config.Interval;
                mc += 1;
                rt = (int) mc;
            }

            return rt >= 10 ? rt.ToString() : "0" + rt; //;
        }
        ...
        private bool SaveRecord(string QiHao, string OpenCode, bool IsYesterdayRecord)
        {
            if (!string.IsNullOrWhiteSpace(QiHao) && !string.IsNullOrWhiteSpace(OpenCode))
            {
                ...
                if (IsYesterdayRecord)
                    model.OpenTime = CommonHelper.GenerateGXKL10FYesterdayOpenTime(Config, QiHao);
                else
                    model.OpenTime = CommonHelper.GenerateGXKL10FTodayOpenTime(Config, QiHao);
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.GuangXiKL10F;
