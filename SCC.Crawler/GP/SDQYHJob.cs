...
namespace SCC.Crawler.GP
{
    ...
    public class SDQYHJob : IJob
    {
        ...
        {
            var postdata = string.Format("q1={0}&q2={1}&method=tjtQyhNew", string.Empty, string.Empty);
            return GetOpenListFromBackUrl(postdata);
        }
        ...
        {
            var yesterdayFirst = CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyMMdd") + "K001";
            var yesterdayLast = CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyMMdd") + "K" +
                                Config.TimesPerDay.ToString().PadLeft(3, '0');
            var postdata = string.Format("q1={0}&q2={1}&method=tjtQyhNew", yesterdayFirst, yesterdayLast);
            return GetOpenListFromBackUrl(postdata, false);
        }
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(string postdata, bool IsToday = true)
        {
            ...
            {
                var HtmlResource =
                    NetHelper.GetUrlResponse(Config.BackUrl, "POST", postdata, Encoding.GetEncoding("gb2312"));
                if (HtmlResource == null) return result;

                var doc = new HtmlDocument();
                doc.LoadHtml(HtmlResource);
                var tables = doc.DocumentNode.SelectNodes("//table");
                if (tables.Count < 2) return result;
                var trs = tables[1].ChildNodes.Where(node => node.Name == "tr").ToList();
                var matchQiHao = string.Empty;
                var matchKJHaoMa = string.Empty;
                List<HtmlNode> tds = null;
                for (var i = 1; i < trs.Count - 1; i++) //第一行是表头，最后一行也是表头内容
                {
                    ...
                    matchQiHao = tds[0].InnerText.Trim().Replace("K", "");
                    matchKJHaoMa = tds[3].InnerText.Trim().Replace(" ", ",");
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.ShanDongQYH),
            ...
            {
                log.Error(GetType(),
                    string.Format("【{0}】通过备用站点抓取开奖列表时发生错误，错误信息【{1}】", Config.Area + currentLottery, ee.Message));
            }
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.ShanDongQYH;
