    ...
    public class NMG11X5Job : IJob
    {
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.NeiMengGu11x5),
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                var tables = doc.DocumentNode.SelectNodes("/html/body/div[2]/div/div[2]/table");
                ...
                foreach (var item in tables[0].ChildNodes)
                    if (item.Name == "tr")
                    {
                        ...
                        var openCodeAry = new List<string>();
                        foreach (var item2 in item.ChildNodes)
                            if (item2.Name == "td")
                            {
                                if (item2.InnerText.Contains("期") &&
                                    item2.GetAttributeValue("style", "").Contains("width:60px;"))
                                    num = item2.InnerText.Replace('期', ' ').Trim();
                                if (item2.GetAttributeValue("style", "").Contains("width:125px;"))
                                    openCodeAry.Add(item2.InnerText.Trim());
                            }

                        if (openCodeAry.Count == 5)
                            result.Add(CommonHelper.SCCSysDateTime.Year % 2000 + num,
                                CommonHelper.TRHandleCode(string.Join(",", openCodeAry)));
                    }
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.NeiMengGu11x5;
