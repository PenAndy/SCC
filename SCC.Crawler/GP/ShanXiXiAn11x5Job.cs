...
namespace SCC.Crawler.GP
{
    ...
    public class ShanXiXiAn11x5Job : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        public ShanXiXiAn11x5Job()
        {
            log = new LogHelper();
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(string url, bool IsToday = true)
        {
            ...
            try
            {
                ...
                foreach (XmlNode xmlnode in records)
                {
                    ...
                    matchQiHao = xmlnode.Attributes["expect"].Value.Trim();
                    matchKJHaoMa = xmlnode.Attributes["opencode"].Value.Trim();
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.ShanXiXiAn11x5),
                ...
                if (data == null)
                {
                    log.Warn(GetType(), string.Format("【{0}】请求接口太频繁，返回了空对象。", Config.Area + currentLottery));
                    return result;
                }
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.ShanXiXiAn11x5;
