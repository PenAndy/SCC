    ...
    public class JS11X5Job : IJob
    {
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                var reg = new Regex(@"<table[\s\S]*?table>", RegexOptions.IgnoreCase);
                var mc = reg.Matches(HtmlResource);
                var matchQiHao = string.Empty;
                var matchKJHaoMa = string.Empty;
                if (mc.Count == 2 && mc[1].Success)
                {
                    var kjTable = mc[1].Value.Replace("\\", "");
                    if (string.IsNullOrWhiteSpace(kjTable)) return result;
                    var doc = new HtmlDocument();
                    doc.LoadHtml(kjTable);
                    var table = doc.DocumentNode.SelectSingleNode("//table");
                    if (table == null) return result;
                    var trs = table.ChildNodes.Where(node => node.Name == "tr").ToList();
                    for (var i = 2; i < trs.Count; i++) //第一行是表头
                    {
                        ...
                        if (matchQiHao == "期号" && matchKJHaoMa == "开奖号") break;
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.JiangSu11X5),
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.JiangSu11X5;
