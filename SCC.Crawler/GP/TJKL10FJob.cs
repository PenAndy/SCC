...
namespace SCC.Crawler.GP
{
    ...
    public class TJKL10FJob : IJob
    {
        ...
        {
            return GetOpenListFromBackUrl(string.Format(Config.BackUrl, 100));
        }
        ...
        {
            return GetOpenListFromBackUrl(string.Format(Config.BackUrl, 200), false);
        }
        ...
        private Dictionary<string, string> GetOpenListFromBackUrl(string url, bool IsToday = true)
        {
            ...
            try
            {
                ...
                var reg = new Regex(@"table_add_one_tr\(""(.*?)"", ""(.*?)""\);");
                var mc = reg.Matches(HtmlResource);
                var matchQiHao = string.Empty;
                var matchKJHaoMa = string.Empty;
                foreach (Match m in mc)
                {
                    if (!m.Success) continue;
                    matchQiHao = m.Result("$1");
                    matchQiHao = matchQiHao.Substring(2);
                    matchKJHaoMa = m.Result("$2");
                    if (string.IsNullOrEmpty(matchKJHaoMa)) continue;
                    matchKJHaoMa = matchKJHaoMa.Replace("|", ",");
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.TianJinKL10F),
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.TianJinKL10F;
