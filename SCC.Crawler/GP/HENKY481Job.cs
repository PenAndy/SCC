...
namespace SCC.Crawler.GP
{
    ...
    public class HENKY481Job : IJob
    {
        ...
        {
            log = new LogHelper();
            services = IOC.Resolve<IOpen4Code>();
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.HeNanKY481),
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            ...
            try
            {
                ...
                var requestUrl = string.Format(Config.BackUrl, 1) + "&moreList=";
                var HtmlResource = NetHelper.GetHeNan481UrlResponse(requestUrl);
                ...
                for (var i = 2; i < trs.Count - 1; i++) //第一二行为表头,最后一行为页码
                {
                    ...
                    matchKJHaoMa = string.Format("{0},{1},{2},{3}", tds[2].InnerText.Trim(), tds[3].InnerText.Trim(),
                ...
                for (var k = 1; k <= 6; k++)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(HtmlResource))
                    {
                        ...
                        for (var i = 2; i < trs.Count - 1; i++) //第一二行为表头,最后一行为页码
                        {
                            ...
                            if (tds.Count < 6) continue;
                            matchQiHao = tds[1].InnerText.Replace("期", string.Empty).Trim();
                            matchKJHaoMa = string.Format("{0},{1},{2},{3}", tds[2].InnerText.Trim(),
        ...
        private bool SaveRecord(string QiHao, string OpenCode, bool IsYesterdayRecord)
        {
            if (!string.IsNullOrWhiteSpace(QiHao) && !string.IsNullOrWhiteSpace(OpenCode))
            {
                ...
                return services.AddOpen4Code(currentLottery, model);
            }
        ...
        ///     数据服务
        /// </summary>
        private readonly IOpen4Code services;

        /// <summary>
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.HeNanKY481;
