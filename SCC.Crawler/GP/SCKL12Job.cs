    ...
    public class SCKL12Job : IJob
    {
        ...
        private void CheckingYesterdayTheLotteryData()
        {
            ...
            if (FailedQiHaoList.Count > 0)
            {
                ...
                //    if (email.AddEmail(Config.Area + currentLottery, fQiHao, CommonHelper.GenerateYesterdayOpenTime(Config, fQiHao)))
                //        log.Error(this.GetType(), CommonHelper.GetJobLogError(Config, fQiHao));
                //}
            }
        }
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            ...
            try
            {
                ...
                if (tables == null || tables.Count < 3) return result;
                var matchQiHao = string.Empty;
                var matchKJHaoMa = string.Empty;
                for (var t = 0; t < tables.Count - 1; t++)
                {
                    var tbody = tables[t].ChildNodes.FirstOrDefault(node => node.Name == "tbody");
                    ...
                    for (var i = 0; i < trs.Count; i++)
                    {
                        ...
                        if (tds.Count < 2 || tds[0].InnerText.Trim() == "--" || tds[1].InnerText.Trim() == "--")
                            continue;
                        matchQiHao = tds[0].InnerText.Trim();
                        matchKJHaoMa = tds[1].InnerText.Trim();
            ...
            {
                log.Error(GetType(),
                    string.Format("【{0}】通过备用站点抓取今日最新开奖列表时发生错误，错误信息【{1}】", Config.Area + currentLottery, ex.Message));
            }
        ...
        private Dictionary<string, string> GetYesterdayOpenListFromBackUrl()
        {
            ...
            try
            {
                var yesterdayFirstQiHao = CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyyyMMdd") + "001";
                var yesterdayLastQiHao = CommonHelper.SCCSysDateTime.AddDays(-1).ToString("yyyyMMdd") +
                                         Config.TimesPerDay.ToString().PadLeft(3, '0');
                var requestUrl =
                    string.Format(
                        "http://zsfx.scflcp.com/cjw12x5/view/12x5_jiben.php?startqi={0}&endqi={1}&searchType=9",
                        yesterdayFirstQiHao, yesterdayLastQiHao);
                ...
                if (!string.IsNullOrWhiteSpace(HtmlResource))
                {
                    var doc = new HtmlDocument();
                    doc.LoadHtml(HtmlResource);
                    var tbody = doc.GetElementbyId("pagedata");
                    ...
                    for (var i = 0; i < trs.Count; i++)
                    {
                        ...
                        matchQiHao = tds[1].InnerText.Trim().Substring(2);
                        matchKJHaoMa = tds[2].InnerText.Trim().Replace(" ", ",");
                        if (!result.ContainsKey(matchQiHao))
                            result.Add(matchQiHao, CommonHelper.TRHandleCode(matchKJHaoMa));
                    }
                }
            ...
            {
                log.Error(GetType(),
                    string.Format("【{0}】通过备用站点抓取昨日开奖列表时发生错误，错误信息【{1}】", Config.Area + currentLottery, ex.Message));
            }
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.SiChuanKL12),
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.SiChuanKL12;
