...
namespace SCC.Crawler.GP
{
    ...
    public class GuiZhou11X5Job : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        public GuiZhou11X5Job()
        {
            log = new LogHelper();
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                {
                    CheckingYesterdayTheLotteryData(); //自检数据
        ...
        /// <returns></returns>
        private List<GuiZhou11x5Entity> GetDocByMainUrl(bool IsToday = true)
        {
            var list = new List<GuiZhou11x5Entity>();
            try
            {
                var time = DateTime.Now;
                var arg = new OpenCaiApiArg
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.GuiZhou11x5),
        ...
        /// <param name="IsToday"></param>
        /// <returns></returns>
        private List<GuiZhou11x5Entity> GetDocByBackUrl(bool IsToday = true)
        {
            var list = new List<GuiZhou11x5Entity>();
            try
            {
                var date = CommonHelper.SCCSysDateTime.ToSCCDateTimeString3();
                if (!IsToday) date = CommonHelper.SCCSysDateTime.AddDays(-1).ToSCCDateTimeString3();
                var startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
                var timeStamp1 = (long) (DateTime.Now - startTime).TotalSeconds; // 相差秒数
                var timeStamp2 = timeStamp1 + new Random().Next(100, 1000);
                //http://www.17mcp.com/out/json/kjgaopinkuayu.do?mark=list&type=3&pubdate=2017-11-19&callbackparam=jsonp1511164496643&_=1511166091893
                //string url = string.Format(@"{0}mark=list&type=3&pubdate={1}&callbackparam=jsonp{2}&_={3}",Config.MainUrl, date, timeStamp1, timeStamp2);
                var url = string.Format(@"{0}mark=list&type=3&pubdate={1}", Config.BackUrl, date);
                var HtmlResource = NetHelper.GetUrlResponse(url, "POST", "1");
                HtmlResource = HtmlResource.Replace("null", "");
                HtmlResource = HtmlResource.Replace("(", "");
                HtmlResource = HtmlResource.Replace(")", ""); // 去掉不能用的部分


                if (HtmlResource == null) return list;

                var obj = HtmlResource.JsonToEntity<JArray>();
                for (var i = 0; i < obj.First["kjgplist"].Count(); i++)
                {
                    var tmp = new GuiZhou11x5Entity
                    {
                        Id = int.Parse(obj.First["kjgplist"][i]["qihao"].ToString()),
                        OpenCode = CommonHelper.TRHandleCode(obj.First["kjgplist"][i]["haoma"].ToString()),
                        OpenTime = GetOpenTimeByPeriodNum(int.Parse(obj.First["kjgplist"][i]["qihao"].ToString()))
                    };
                    list.Add(tmp);
                }
        ...
        /// <returns></returns>
        public bool SaveRecord(GuiZhou11x5Entity data)
        {
    ...
    public class GuiZhou11x5Entity
    {
