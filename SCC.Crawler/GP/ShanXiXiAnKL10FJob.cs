...
namespace SCC.Crawler.GP
{
    ...
    public class ShanXiXiAnKL10FJob : IJob
    {
        #region 旧代码

    //    public ShanXiXiAnKL10FJob()
    //    {
    //        log = new LogHelper();
    ...
    //            if (matchItem.Id != null && SaveRecord(matchItem))
    //            {
    //                //处理成功写入日志
    ...
    //    /// <returns></returns>
    //    private List<ShanXiXiAnKL10FEntity> GetDocByBackUrl(bool IsToday = true)
    //    {
    //        var list = new List<ShanXiXiAnKL10FEntity>();
    //        try
    //        {
    //            var dayParam = "today";
    //            if (!IsToday) dayParam = "yesterday";
    //            var param = string.Format(@"?action=chart&date={0}&random=0.20514743565046767&id=1007&async=true",
    //                dayParam);
    //            var htmlResource = NetHelper.GetUrlResponse(Config.BackUrl + param); //Config.MainUrl
    //            if (string.IsNullOrWhiteSpace(htmlResource))
    //            {
    //                log.Warn(GetType(), string.Format("【{0}】通过接口抓取开奖列表未获取到数据。", Config.Area + currentLottery));
    //                return list;
    //            }
    ...
    //                {
    //                    Id = int.Parse(data["data"][i]["dateNumber"].ToString()),
    //                    OpenTime = DateTime.Parse(data["data"][i]["dateTime"].ToString())
    //                };
    //                for (var j = 0; j < data["data"][i]["list"].Count(); j++)
    //                    codelist.Add(data["data"][i]["list"][j].ToString());
    //                tmp.OpenCode = CommonHelper.TRHandleCode(string.Join(",", codelist));
    //                list.Add(tmp);
    //            }
    ...
    //    /// <param name="IsToday"></param>
    //    /// <returns></returns>
    //    private List<ShanXiXiAnKL10FEntity> GetDocByMainUrl(bool IsToday = true)
    //    {
    //        var list = new List<ShanXiXiAnKL10FEntity>();
    //        try
    //        {
    ...
    //            {
    //                code = EnumHelper.GetLotteryCode(SCCLottery.ShanXiXiAnKL10F),
    ...
    //                    var tmp = new ShanXiXiAnKL10FEntity
    //                    {
    //                        Id = int.Parse(data.data[i].GetTermStr().Remove(6, 1)),
    ...
    //    /// <returns></returns>
    //    public bool SaveRecord(ShanXiXiAnKL10FEntity data)
    //    {
    ...
    //    ///     当前彩种
    //    /// </summary>
    //    private SCCLottery currentLottery => SCCLottery.ShanXiXiAnKL10F;
    ...
    /////     开奖号实体
    ///// </summary>
    //public class ShanXiXiAnKL10FEntity
    //{
...

    {
        ...
        try
        {
            ...
            foreach (XmlNode xmlnode in records)
            {
                ...
                var qihao= xmlnode.Attributes["expect"].Value.Substring(2);
                var nian = day.Substring(2);
                var num = nian + "0" + qihao;
    ...
    /// <returns></returns>
    private bool SaveRecord(ShanXiXiAnKL10FEntity data)
    {
        var model = new OpenCode8Model();
        model.Term = Convert.ToInt64(data.Id); //期号
        var haoMaArray = data.OpenCode.Split(',');
