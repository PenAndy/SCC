...
namespace SCC.Crawler.GP
{
    ...
    public class GuiZhouK3Job : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        ...
        /// <param name="IsToday"></param>
        /// <returns></returns>
        private List<GuiZhouK3Entity> GetDocByMainUrl(bool IsToday = true)
        {
            var list = new List<GuiZhouK3Entity>();
            try
            {
                var time = DateTime.Now;
                var arg = new OpenCaiApiArg
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.GuiZhouK3),
        ...
        /// <returns></returns>
        private bool SaveRecord(GuiZhouK3Entity data)
        {
        ...
        /// <param name="IsToday"></param>
        /// <returns></returns>
        //private List<GuiZhouK3Entity> GetDocByBackUrl(Boolean IsToday = true)

        //{
        //    List<GuiZhouK3Entity> list = new List<GuiZhouK3Entity>();
        //    try
        //    {
        //        string day = DateTime.Now.ToString("yyyyMMdd");
        //        if (!IsToday)
        //        {
        //            day = DateTime.Now.AddDays(-1).ToString("yyyyMMdd");
        //        }
        //        string url = string.Format(Config.BackUrl, day);
        //        // "http://kaijiang.500.com/static/info/kaijiang/xml/fjk3/20171127.xml";
        ...
        //            {
        //                if (xmlnode.Attributes["expect"] == null || string.IsNullOrWhiteSpace(xmlnode.Attributes["expect"].Value) || xmlnode.Attributes["opencode"] == null || string.IsNullOrWhiteSpace(xmlnode.Attributes["opencode"].Value)) continue;
        //                string num = xmlnode.Attributes["expect"].Value.ToString();
        //                string opencode = xmlnode.Attributes["opencode"].Value;
        //                string opentime = xmlnode.Attributes["opentime"].Value;
        //                GuiZhouK3Entity tmp = new GuiZhouK3Entity()
        //                {
        //                    Id = int.Parse(num),
        //                    OpenCode = opencode,
        ...
        //            var dbdata = services.GetListIn(currentLottery, IsToday).ToDictionary(w => w.Term.ToString(), w => w.GetCodeStr());
        //            checkDataHelper.CheckData(dbdata, list.ToDictionary(w => w.Id.ToString(), w => w.OpenCode), Config.Area, currentLottery);

        //    }
        //    catch (Exception ee)
        //    {

        //        log.Error(this.GetType(), string.Format("【{0}】通过站点抓取开奖列表时发生错误，错误信息【{1}】", Config.Area + currentLottery, ee.Message));
        //    }
        //    return list;
        //}
        ...
        #region   爬取爱彩乐网站数据
        private List<GuiZhouK3Entity> GetDocByBackUrl(bool IsToday = true)
        {
            var list = new List<GuiZhouK3Entity>();
            try
            {
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.GuiZhouK3;
