...
namespace SCC.Crawler.GP
{
    ...
    public class FJ11X5Job : IJob
    {
        ...
        ///     (福建体彩网)
        /// </summary>
        public void DoTodayJobByBackUrl()
        {
        ...
        private Dictionary<string, string> GetTodayOpenListFromBackUrl()
        {
            ...
            try
            {
                var HtmlResource = NetHelper.GetUrlResponse(string.Format(Config.BackUrl, 1));
                if (HtmlResource == null) return result;

                var s = new Regex(@"<table[\s\S]*?</table>");
                var m = s.Match(HtmlResource);
                if (m.Success)
                {
                    var tbString = m.Value.Replace("</div>", "</span>");
                    var doc = new HtmlDocument();
                    doc.LoadHtml(tbString);
                    ...
                    for (var i = 3; i < trs.Count; i++) //第一二三行为表头
                    {
                        if (trs[i].Attributes["class"] != null && trs[i].Attributes["class"].Value == "numclick") break;
            ...
            {
                log.Error(GetType(),
                    string.Format("【{0}】通过主站点抓取今日最新开奖列表时发生错误，错误信息【{1}】", Config.Area + currentLottery, ex.Message));
            }
        ...
        private Dictionary<string, string> GetYesterdayOpenListFromBackUrl()
        {
            ...
            try
            {
                for (var k = 1; k <= 2; k++)
                {
                    var HtmlResource = NetHelper.GetUrlResponse(string.Format(Config.BackUrl, k));
                    if (HtmlResource == null) return result;
                        ...
                        if (m.Success)
                        {
                            ...
                            for (var i = 3; i < trs.Count; i++) //第一二三行为表头
                            {
                                if (trs[i].Attributes["class"] != null &&
                                    trs[i].Attributes["class"].Value == "numclick") break;
                                tds = trs[i].ChildNodes.Where(R => R.Name.ToLower() == "td").ToList();
                                if (tds.Count < 6) continue;
                                matchQiHao = tds[0].InnerText.Trim();
                                matchKJHaoMa = string.Format("{0},{1},{2},{3},{4}", tds[1].InnerText.Trim(),
        ...
        private Dictionary<string, string> GetDocByMainUrl(bool IsToday = true)
        {
            ...
            try
            {
                ...
                {
                    code = EnumHelper.GetLotteryCode(SCCLottery.FuJian11x5),
        ...
        private void CheckData(Dictionary<string, string> dbData, Dictionary<string, string> newData)
        {
            try
            {
                foreach (var item in dbData)
                {
                    ...
                    if (data.Key != null)
                        if (data.Value != item.Value)
                            email.AddEmail(Config.Area + currentLottery, data.Key, DateTime.Now, "数据验证失败");
                }
            }
            catch (Exception ex)
            {
                log.Error(GetType(),
                    string.Format("【{0}】数据验证发生错误，错误信息【{1}】", Config.Area + currentLottery, ex.Message));
                //报错处理
            }
        }
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.FuJian11x5;
