...
namespace SCC.Crawler.DT
{
    ...
    public class HuBei30X5Job : IJob
    {
        ...
        private List<OpenCode5DTModel> GetOpenListFromMainUrl(string mainUrl)
        {
            ...
            try
            {
                ...
                for (var i = 2; i < trs.Count; i++) //第一二行为表头
                {
                    ...
                    nodeA = tds[10].ChildNodes.Where(n => n.Name == "a").FirstOrDefault();
                ...
                var date = dbdata.ToDictionary(w => w.Term.ToString(), w => w.GetCodeStr());
                checkDataHelper.CheckData(date, result.ToDictionary(w => w.Term.ToString(), w => w.GetCodeStr()),
        ...
        {
            var TrjeMoney = nodes[3].InnerText.Replace("万", "").Replace(" ", "0").Trim();
            var entity = new KaijiangDetailsEntity
            {
                Gdje = "",
                Trje = TrjeMoney == "" ? "0" : (double.Parse(TrjeMoney) * 10000).ToString()
            };
            //TODO 
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.HuBei30X5;
