...
namespace SCC.Crawler.DT
{
    ...
    public class GuangDong26X5Job : IJob
    {
        /// <summary>
        ///     构造函数
        /// </summary>
        public GuangDong26X5Job()
        {
            log = new LogHelper();
        ...
        public void Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                if (CommonHelper.CheckTodayIsOpenDay(Config) && CommonHelper.SCCSysDateTime.Hour > 10)
                {
                    DoMainUrl();
                    DoBackUrl();
                }
        ...
        private List<OpenCode5DTModel> GetOpenListFromMainUrl(string mainUrl)
        {
            ...
            try
            {
                ...
                for (var i = 2; i < trs.Count; i++) //第一二行为表头
                {
                    ...
                    model.OpenTime = Convert.ToDateTime(tds[7].InnerText);
        ...
        private List<OpenCode5DTModel> GetOpenListFromBackUrl()
        {
            ...
            try
            {
                ...
                {
                    log.Warn(GetType(), string.Format("【{0}】通过备用站抓取开奖列表未获取到数据。", Config.Area + currentLottery));
                    return result;
                }
                ...
                {
                    var model = new OpenCode5DTModel();
                    tds = trs[i].ChildNodes.Where(S => S.Name.ToLower() == "td").ToList();
                    if (tds.Count < 3) continue;


                    term = tds[0].InnerText.Trim();
                    if (!term.StartsWith(CommonHelper.SCCSysDateTime.Year.ToString())) continue;

                    model.Term = Convert.ToInt64(term);
                    openCodeString = tds[1].GetAttributeValue("luckyno", "");
                    if (openCodeString.Length < 10) continue;
        ...
        ///     当前彩种
        /// </summary>
        private SCCLottery currentLottery => SCCLottery.GuangDong26X5;
