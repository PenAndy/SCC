...
{
    public class Open9CodeServices : BaseServices, IOpen9Code
    {
        /// <summary>
        /// 获取最新一条记录
        /// </summary>
        ...
        /// <returns></returns>
        public OpenCode9Model GetLastItem(SCCLottery lottery)
        {
            ...
            {
                var result = LoadData<OpenCode9Model>(ds.Tables[0].Rows[0]);
                return result;
            }
            return null;
        }
        ...
        /// <param name="lottery">彩种名称</param>
        /// <returns></returns>
        public List<OpenCode9Model> GetListIn(SCCLottery lottery, bool IsToday)
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                var data = LoadDataList<OpenCode9Model>(ds.Tables[0]);
                return data;
            }
            else
            {
                return new List<OpenCode9Model>();
            }
        ...
        /// <returns></returns>
        public bool AddOpen9Code(SCCLottery lottery, OpenCode9Model model)
        {
            var TableName = EnumHelper.GetSCCLotteryTableName(lottery);
        ...
        private static string AddItemSql = @"IF NOT EXISTS(SELECT TOP 1 1 FROM {0} WHERE Term = @Term)
                                            BEGIN
	                                            INSERT INTO {0}(Term,OpenCode1,OpenCode2,OpenCode3,OpenCode4,OpenCode5,OpenCode6,OpenCode7,OpenCode8,OpenCode9,OpenTime,Addtime,ID)
                                                SELECT @Term,@OpenCode1,@OpenCode2,@OpenCode3,@OpenCode4,@OpenCode5,@OpenCode6,@OpenCode7,@OpenCode8,@OpenCode9,@OpenTime,GETDATE(),@ID
                                            END";
